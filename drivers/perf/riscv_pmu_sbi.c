...
static irqreturn_t pmu_sbi_ovf_handler(int irq, void *dev)
{
	...
	regs = get_irq_regs();
	/*
	 * When Sspesa is available and a counter overflows we save the PC and the metadata that is
	 * captured by the hardware. If the corresponding event supported precise attribution then
	 * the PC value is precise and related to that instruction which caused the
	 * overflow, otherwise it is 'best-effort'.
	 */
	if (sspesa_available) {
	...
	for_each_set_bit(lidx, cpu_hw_evt->used_hw_ctrs, RISCV_MAX_COUNTERS) {
		...
		if (sspesa_available && hidx == sample_cntrid) {
			...
			/*
			 * Append the implementation specific `shpmsdata` value if the userspace
			 * requested for PERF_SAMPLE_RAW sample type.
			 */
